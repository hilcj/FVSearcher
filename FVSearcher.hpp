...
class EQ_parallel{
	...
	public:
		...
		void process(){
			...
			}while(E.size() != 0); // Check whether all nodes were properly processed
...
// Methods
	...
	void full_atomwise_old(int flat) {
...
/*	    // Split
		switch (arg.num_threads){
			case 1: {i_split = 1; j_split = 1; k_split = 1; break; }
			case 2: {i_split = 2; j_split = 1; k_split = 1; break; }
			case 3: {i_split = 3; j_split = 1; k_split = 1; break; }
			case 4: {i_split = 2; j_split = 2; k_split = 1; break; }
			case 5: {i_split = 5; j_split = 1; k_split = 1; break; }
			case 6: {i_split = 2; j_split = 3; k_split = 1; break; }
			case 7: {i_split = 7; j_split = 1; k_split = 1; break; }
			case 8: {i_split = 2; j_split = 2; k_split = 2; break; }
			case 9: {i_split = 3; j_split = 3; k_split = 1; break; }
			case 10: {i_split = 2; j_split = 5; k_split = 1; break; }
			case 11: {i_split = 11; j_split = 1; k_split = 1; break; }
			case 12: {i_split = 2; j_split = 2; k_split = 3; break; }
			case 13: {i_split = 13; j_split = 1; k_split = 1; break; }
			case 14: {i_split = 2; j_split = 7; k_split = 1; break; }
			case 15: {i_split = 3; j_split = 5; k_split = 1; break; }
			case 16: {i_split = 2; j_split = 2; k_split = 4; break; }
			case 17: {i_split = 17; j_split = 1; k_split = 1; break; }
			case 18: {i_split = 2; j_split = 3; k_split = 3; break; }
			case 19: {i_split = 19; j_split = 1; k_split = 1; break; }
			case 20: {i_split = 2; j_split = 2; k_split = 5; break; }
			case 21: {i_split = 3; j_split = 7; k_split = 1; break; }
			case 22: {i_split = 2; j_split = 11; k_split = 1; break; }
			case 23: {i_split = 23; j_split = 1; k_split = 1; break; }
			case 24: {i_split = 2; j_split = 3; k_split = 4; break; }
		}
		i_size = int(ceil(1.0 * im / i_split));
		j_size = int(ceil(1.0 * jm / j_split));
		k_size = int(ceil(1.0 * km / k_split));
*/
		int m = 10;
		i_size = m; j_size = m; k_size = m;
		i_split = int(ceil(1.0 * im / i_size));
		j_split = int(ceil(1.0 * jm / j_size));
		k_split = int(ceil(1.0 * km / k_size));
	...
	void CCL_serial_two_pass(int flat) {
	    ...
	    int ibegin = i * i_size;
	    int iend = fmin((i + 1) * i_size, im);
	    int jbegin = j * j_size;
	    int jend = fmin((j + 1) * j_size, jm);
	    int kbegin = k * k_size;
	    int kend = fmin((k + 1) * k_size, km);
	    		...
	    		for (int kk = kbegin; kk < kend; kk++) {
	    			...
	    			if (M_full[flat_cur]) {
						...
						if (nl.size() == 0) { 
							...
							temp = eq.assign_new_label();

						} else {

							// Find the minimal label and apply
							int min_label = i_size * j_size * k_size;
...
public:
			...
			if (rfile.is_open()) {
				...
				while( getline (rfile, line)) {
					if (line.compare(0,4,"ATOM") == 0) {
						...
						if (cur.x + cur.r > roi.xb and 
							cur.x - cur.r < roi.xe and 
							cur.y + cur.r > roi.yb and 
							cur.y - cur.r < roi.ye and 
							cur.z + cur.r > roi.zb and 
							cur.z - cur.r < roi.ze){
							A.push_back(cur);
						}
					}
	...
	void build_array_occupancy_full_pointwise(){
		...
		double t2 = omp_get_wtime();
		cout <<"TTT:  - "<<arg.num_threads<<" cores "<<fixed << std::setprecision(3)<<t2-t1<<" seconds"<<endl;
	}
	...
	void build_array_occupancy_full_atomwise(){
		...
		double t2 = omp_get_wtime();
		cout <<"TTIME:  - "<<arg.num_threads<<" cores "<<fixed << std::setprecision(3)<<t2-t1<<" seconds"<<endl;
		cout <<"TCUM_:  - "<<arg.num_threads<<" cores "<<fixed << std::setprecision(3)<<t_cum<<" seconds"<<endl;
	}
	...
	void parallel_CCL_serial_block() {
		...
		#pragma omp parallel for num_threads(arg.num_threads)
	    for (int flat = 0; flat < iljlkl; flat++) {
	    	CCL_serial_two_pass(flat);
	    }
	    double t2 = omp_get_wtime();
	    cout <<"CORETIME:  - serialCCL "<<fixed << std::setprecision(3)<<t2-t1<<" seconds"<<endl;
	...
	void parallel_CCL_apply_equivalencies() {
	    ...
	    // Extract equivalencies
	    #pragma omp parallel num_threads(arg.num_threads) default(shared)
		{
			// Copy global to local
			map < pair <int, int>, int > local2global; 
	    	int num_labels_global = EQ.get_num_labels_global() + 1;
	    	int * P = new int [num_labels_global];

			local2global = EQ.local2global;
			for (int i = 1; i < num_labels_global; i++){
				P[i] = EQ.P[i];
			}
		    ...
		    for (int flat = 0; flat < imjmkm; flat++) {
		    	if (M_full[flat]){
			    	...
			    	L_full[flat] = P[local2global[pair <int, int> (flat_block_id, L_full[flat])]];
			    }
		    }
		}
	}
